...
// Emit when driver proposes a final price
export const emitPriceProposed = (order: any): void => {
  if (!io) {
    console.warn('Socket server not initialized when emitting price proposal');
    return;
  }
  ...
  console.log(`💰 Price proposed for order ${order?._id}: ${order?.finalPrice} NIS`);
  ...
  // Also send order-updated to keep both apps in sync
  emitToUserRoom(customerId, 'order-updated', order);
  emitToUserRoom(driverId, 'order-updated', order);
};

// Emit when user accepts or rejects the proposed price
export const emitPriceResponse = (order: any, accepted: boolean): void => {
  if (!io) {
    console.warn('Socket server not initialized when emitting price response');
    return;
  }
  ...
  const responseType = accepted ? 'price-accepted' : 'price-rejected';
  console.log(`💰 Price ${accepted ? 'accepted' : 'rejected'} for order ${order?._id}`);

  // Notify driver about the response
  emitToUserRoom(driverId, responseType, {
    orderId: order?._id?.toString() ?? order?.id,
    accepted: accepted,
    finalPrice: order?.finalPrice,
    order: order,
