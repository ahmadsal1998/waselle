...
class ApiService {
  ...
  static Map<String, dynamic> _parseResponse(http.Response response) {
    if (response.body.isEmpty) {
      throw Exception('Empty response from server. Status: ${response.statusCode}. Make sure the backend server is running on port 5001.');
    }
    ...
    try {
      ...
      if (decoded is Map<String, dynamic>) {
        ...
        throw Exception('Invalid response format from server. Expected JSON object but got: ${decoded.runtimeType}');
      }
      ...
      final preview = response.body.length > 100 
        ...
        : response.body;
      throw Exception('Invalid JSON response from server. Status: ${response.statusCode}. Error: ${e.message}. Response preview: $preview');
    }
  }
  ...
  static Future<Map<String, dynamic>> register({
    ...
    try {
      ...
      if (e is http.ClientException || 
        ...
        throw Exception('Unable to connect to server. Please check your internet connection or ensure the backend server is running on port 5001.');
      }
      rethrow;
    }
  }
  ...
  static Future<Map<String, dynamic>> getOrderById(String orderId) async {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        throw Exception('Failed to fetch order with status ${response.statusCode}');
      }
    } catch (e) {
      if (e is Exception) rethrow;
  ...
  static Future<Map<String, dynamic>> getCurrentUser() async {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        throw Exception('Failed to get current user with status ${response.statusCode}');
      }
    } catch (e) {
      if (e is Exception) rethrow;
