# Android FCM Token Retrieval Fix - User App

## Problem
Some Android devices were not receiving push notifications because FCM tokens were not being retrieved or saved to the backend. Server logs showed users with no FCM token, causing notifications to be skipped.
...
5. **Token not saved after authentication** - No mechanism to save token after user logs in
...
- Called automatically after login and authentication check
...
- `savePendingToken()` is now called after:
  - Successful login
  - Successful OTP verification
  - Successful authentication check on app start
...
- [x] Token is saved after authentication (login, OTP verification, auth check)
...
### 3. Test Token After OTP Verification
1. Register/login with phone number
2. Verify OTP
3. Verify token is saved to backend
...
**Solution:**
- Token will be saved automatically after login/OTP verification
...
2. `lib/view_models/auth_view_model.dart`
   - Added FCMService import
   - Added `savePendingToken()` call after login
   - Added `savePendingToken()` call after OTP verification
   - Added `savePendingToken()` call after auth check

### Unchanged Files (Verified Correct)
1. `android/app/src/main/AndroidManifest.xml` - ✅ Correct
2. `android/app/build.gradle.kts` - ✅ Correct (if exists)
3. `android/app/google-services.json` - ✅ Present
4. `lib/main.dart` - ✅ Background handler registered correctly
...
- The user app uses both `FCMService` and `NotificationService`. Both services handle FCM tokens, but `FCMService` is now the primary service with enhanced retry logic.
- Token retrieval works independently of notification permissions - tokens can be retrieved even if user denies notification permissions.
- Tokens are automatically saved after any authentication event (login, OTP verification, auth check).
