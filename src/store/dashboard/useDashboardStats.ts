...
import type { ApiUser, OrderStatus } from '../../types';
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const activeOrders = orders.filter((order) =>
        isStatusIncluded(order.status, ACTIVE_STATUSES)
      );
      const completedOrders = orders.filter((order) =>
        isStatusIncluded(order.status, COMPLETED_STATUSES)
      );
      const pendingOrders = orders.filter((order) =>
        isStatusIncluded(order.status, PENDING_STATUSES)
      );
