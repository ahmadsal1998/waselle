...
# Keystore Setup Script for Wassle Driver App
# This script helps you create a signing keystore for Google Play

set -e

echo "🔐 Setting up signing keystore for Wassle Driver App"
echo ""
...
KEYSTORE_PATH="$HOME/wassle-driver-keystore.jks"
...
if [[ "$KEYSTORE_PATH" == "$HOME"/* ]]; then
    # Try to use ~ instead of full path
    RELATIVE_KEYSTORE="~/${KEYSTORE_PATH#$HOME/}"
else
    RELATIVE_KEYSTORE="$KEYSTORE_PATH"
fi
